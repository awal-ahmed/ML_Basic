...
inp = tf.keras.Input(shape=(IMAGE_HEIGHT, IMAGE_WIDTH, 3))
...
dec = Conv2DTranspose(filters=NUM_CLASSES, kernel_size=2, strides=2, padding='same', kernel_initializer='he_normal', name='fullconv')(dec)
dec = Activation('softmax')(dec)

model = Model(inputs=inp, outputs=dec, name='Enet')
model.save(f'enet_{NUM_CLASSES}.h5')